...
export default function Sun() {
  ...
  `,
    ...
    void main() {
      vec4 p = vec4(vPosition * 1., time*0.05);
      vec4 p1 = vec4(vPosition, time*0.05);

      float spots = max(snoise(p1), 0.);
      float noisy = fbm(p);

      vec3 white = vec3(1.,0.667,0.2);
      vec3 coral = vec3(1.,0.4,0.);

      float fres = Fresnel(eyeVector, vNormal);

      float brightness = noisy * mix(1., spots, 0.7) * 2. + 1.1;
      brightness += fres;

      vec3 col = brigthnessToColor(brightness);
